...
import com.thinkspace.opentalkon.ui.OTOpenTalkDetail;
import com.thinkspace.opentalkon.ui.OTChatRoom;
import com.thinkspace.opentalkon.ui.OTPublicChatRoom;
...
public class ConversationMgr {
	...
	public boolean IsTalkConvForeground(boolean public_room){
		if(public_room){
			return OTOApp.getInstance().IsForeground(OTPublicChatRoom.class.getSimpleName());
		}else{
			return OTOApp.getInstance().IsForeground(OTChatRoom.class.getSimpleName());
		}
	}
	
	public void newMsgNotification(Long room_id, Long sender_id, String msg, boolean public_room){
		if(OTOApp.getInstance().getPref().getSetting_chat_notifiy().getValue() == false) return;
		if(IsTalkConvForeground(public_room) && room_id.equals(getLastJoinedRoomId()) && roomScreenOff == false) return;
		
		Intent intent = null;
		if(public_room){
			intent = new Intent(context, OTPublicChatRoom.class);
		}else{
		...
		String userName = TAUserNick.getInstance().getUserInfo(sender_id);
		setLastNotifiedRoomId(room_id);
		setupNotficationForMsgAnnounce(++talkNotificationCnt, context.getString(R.string.oto_notification_message_popup), userName + " : " + msg,
				Notification.FLAG_AUTO_CANCEL, intent, NOTIFICATION_MSG_ID, true, OTOApp.getInstance().getCacheCtrl().getTalkAlarm(room_id));
	}
	
	public void newCommunityPostNotification(long community_id){
		if(OTOApp.getInstance().getPref().getSetting_opentalk_new_post_notifiy().getValue() == false) return;
		if(hasCommunity(community_id)) return;
		
		Intent intent = new Intent(context, OTOpenTalkRoom.class);
		intent.putExtra("community_id", community_id);
		
		setLastNotifiedCommunityId(community_id);
		setupNotficationForMsgAnnounce(0, context.getString(R.string.oto_community_notification),
				context.getString(R.string.oto_community_notification), Notification.FLAG_AUTO_CANCEL, intent, NOTIFICATION_COMMUNITY_ID, true, true);		
	}
	
	public static enum CommunityNotificationType{
		REPLY,
		LIKE
	}
	
	public void newMineCommunityNotification(CommunityNotificationType type, long community_id, long post_id, String nick_name, long user_id, long id){
		if(type == CommunityNotificationType.REPLY){
			if(OTOApp.getInstance().getPref().getSetting_opentalk_my_reply_notifiy().getValue() == false) return;
		}else if(type == CommunityNotificationType.LIKE){
			if(OTOApp.getInstance().getPref().getSetting_opentalk_my_like_notifiy().getValue() == false) return;
		}
		
		String notifyMsg = null;
		Intent intent = new Intent(context, OTOpenTalkDetail.class);
		intent.putExtra("post_id", post_id);
		if(type == CommunityNotificationType.REPLY){
			intent.putExtra("click_reply", true);
			notifyMsg = String.format(context.getString(R.string.oto_reply_notification), nick_name);
			int tableIdx = OTOApp.getInstance().getCacheCtrl().addNewNotification(nick_name, user_id, TANotification.TYPE_REPLY_MINE, post_id, id);
			intent.putExtra("tableIdx", tableIdx);
			setupNotficationForMsgAnnounce(0, notifyMsg,
					notifyMsg, Notification.FLAG_AUTO_CANCEL, intent, NOTIFICATION_COMMUNITY_ID, true, true);
		}
		if(type == CommunityNotificationType.LIKE){
			notifyMsg = String.format(context.getString(R.string.oto_like_notification), nick_name);
			if(OTOApp.getInstance().getCacheCtrl().hasLikeNotification(user_id, post_id) == false){
				int tableIdx = OTOApp.getInstance().getCacheCtrl().addNewNotification(nick_name, user_id, TANotification.TYPE_LIKE_MINE, post_id, id);
				intent.putExtra("tableIdx", tableIdx);
				setupNotficationForMsgAnnounce(0, notifyMsg,
						notifyMsg, Notification.FLAG_AUTO_CANCEL, intent, NOTIFICATION_COMMUNITY_ID, true, true);
			}
	...
	public void newCommunityReplyNotification(long community_id, long post_id, String nick_name, long user_id, long id){
		if(OTOApp.getInstance().getPref().getSetting_opentalk_reply_notifiy().getValue() == false) return;
		String notifyMsg = null;
		Intent intent = new Intent(context, OTOpenTalkDetail.class);
		intent.putExtra("post_id", post_id);
		intent.putExtra("click_reply", true);
		
		notifyMsg = String.format(context.getString(R.string.oto_reply_notification2), nick_name);
		int tableIdx = OTOApp.getInstance().getCacheCtrl().addNewNotification(nick_name, user_id, TANotification.TYPE_REPLY, post_id, id);
		intent.putExtra("tableIdx", tableIdx);
		setupNotficationForMsgAnnounce(0, notifyMsg,
				notifyMsg, Notification.FLAG_AUTO_CANCEL, intent, NOTIFICATION_COMMUNITY_ID, true, true);
	}
